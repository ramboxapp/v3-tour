...
describe("VTour.vue", () => {
	...
	it("uses default container option", () => {
		...
		expect(wrapper.vm.customOptions.container).toEqual("body");
	});

	it("uses custom container option when provided", () => {
		const wrapper = shallowMount(VTour, {
			propsData: {
				name: "myTestTour",
				steps,
		...
		expect(wrapper.vm.customOptions.container).toEqual("#my-container");
	});
